...
Make Pieria installable and measurable. This phase adds native/JVM packaging, first-run setup, OS service scripts, local
observability, and an evaluation harness that can guide retrieval and prompt changes.
...
7. Build fixture-based evaluation first.
  ...
  - Track extraction precision/recall, retrieval hit rate, ranking quality, answer faithfulness, latency, and token
    usage.
...
- Benchmark adapters may require dataset licensing or manual download steps, so keep local fixtures as the
  always-available quality gate.
