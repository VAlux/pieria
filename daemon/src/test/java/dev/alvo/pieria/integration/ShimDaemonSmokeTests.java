...
/**
 * End-to-end smoke test (phase-4 step 8): the shim's real {@link DaemonClient} + {@link MemoryTools}
 * driven against a REAL daemon booted on a random local port. The embedded SQLite store is the real
 * throwaway DB (application-test.properties), so storage is exercised end-to-end; only the
 * {@link ModelGateway} is faked (no Ollama, no network egress).
 *
 * <p>This complements {@code MemoryToolsTests} (which uses a fake HttpServer): here the daemon's
 * actual REST stack + storage answer the shim, proving the full round trip.
 */
@ActiveProfiles("test")
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class ShimDaemonSmokeTests {

  /** Override the model gateway with a deterministic, network-free fake — keep the real store. */
  @TestConfiguration
  static class FakeModelConfig {
    @Bean("shimSmokeFakeModelGateway")
    @Primary
