...
class EffectiveConfigResolverTests {
  ...
  private static PieriaProperties propertiesWith(PieriaProperties.Retrieval retrieval) {
    return new PieriaProperties(null, null, null, null,
      new PieriaProperties.Ingestion(10000, 2, 4, VerifyMode.ALWAYS,
        1, 0, 0, false, 3, 3, 32, 5, true, 5000, true, 0.70),
      retrieval,
  ...
  private static PieriaProperties.Retrieval overlayFor(PieriaProperties global, DaemonOverrides overrides) {
    ResolvedConfig base = EffectiveConfigResolver.withoutOverrides(global).resolve("unused-profile");

    ConfigOnlyStore store = new ConfigOnlyStore();
    store.configs.put("p1", ConfigCodec.toJson(overrides));
    EffectiveConfigResolver resolver = new EffectiveConfigResolver(global, store);

    ResolvedConfig effective = resolver.resolve("p1");
    assertThat(effective.ingestion()).isEqualTo(base.ingestion()); // untouched by these overrides
