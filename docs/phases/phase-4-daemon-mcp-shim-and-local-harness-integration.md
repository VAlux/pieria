...
Harden Pieria as a local daemon and make it usable from agent harnesses through a thin MCP stdio gateway plus lifecycle
hook assets. Claude Code integration is first-class in this phase, with OpenCode and Codex documented as equivalent
configurations.
...
2. Stabilize REST API behavior.
  ...
  - Add consistent error response bodies for validation, daemon unavailable, model unavailable, and missing memory
    cases.
...
3. Add MCP stdio gateway.
  - Implement the gateway as thin client code in the existing project unless a later packaging need justifies a separate
    artifact.
