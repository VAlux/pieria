...
Shipped: `ingestion.trace` (`TraceEvent`/`CommandSignature`/`TraceRelevanceFilter`/`TraceMemoryFactory`/`TraceGraphBuilder`/`TraceCodeLinker`/`TraceRecipeExtractor`/`TraceIngestionService`) behind an optional `traces` list on `POST /v1/profiles/{name}/ingest`; Claude Code's `PostToolUse` and `PostToolUseFailure` events and Codex's `PostToolUse` event are captured into a local spool that the turn-end hooks drain. Claude's hook event name supplies authoritative success/failure status. Codex only emits post-use events for successful non-shell results, while its Bash hook response currently omits the process exit code; those Bash outcomes remain honestly `unknown` unless a numeric exit code is present. Outcome `event`s are derived deterministically in Java and keyed `trace:outcome:<signature>` so the latest supersedes; procedural `instruction`s are model-derived once per batch and keyed `trace:recipe:<signature>`. Traces ride the existing channels — no new channel — and reach the code channels through `payload.symbolIds`. OpenCode capture is not implemented because its per-tool lifecycle event has not been verified.
