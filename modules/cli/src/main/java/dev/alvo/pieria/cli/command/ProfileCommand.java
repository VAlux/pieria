...
/**
 * {@code pieria profile} — print the Pieria profile slug for a directory, using the same resolution
 * as the gateway and hooks ({@link ProfileResolver}: {@code $PIERIA_PROFILE} → git remote → dir name).
 */
@Command(
  name = "profile",
...
public final class ProfileCommand implements Callable<Integer> {
