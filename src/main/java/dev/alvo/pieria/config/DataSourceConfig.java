...
public class DataSourceConfig {
  ...
  @Bean
  @Primary
  public DataSource dataSource(PieriaProperties properties, VecCapability vecCapability) {
    ...
    loadVecExtensionBestEffort(dataSource, vecCapability);
    return dataSource;
  }
   ...
   */
  private static void loadVecExtensionBestEffort(HikariDataSource dataSource, VecCapability cap) {
    try (Connection conn = dataSource.getConnection(); Statement st = conn.createStatement()) {
      // Try the common entry-point names; the first that succeeds wins.
      boolean loaded = tryLoad(st, "vec0")
        || tryLoad(st, "vec")
        || tryLoad(st, "sqlite_vec")
        || tryLoadAuto(st);
      if (loaded && probeVec(st)) {
        cap.markLoaded();
        log.info("sqlite-vec extension loaded; embedded vector search enabled.");
      } else {
        log.warn("sqlite-vec extension not available; vector search disabled "
          + "(FTS + keyed lookup still work). Install the native sqlite-vec library to enable it.");
      }
    } catch (Exception e) {
