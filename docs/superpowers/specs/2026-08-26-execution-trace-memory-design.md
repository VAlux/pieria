...
Trace memories emit a `GraphFragment` (`command`, `tool`, `test`, `file`, `module` entities) into
...
`TraceStatus` is a sibling enum in the same package (`dev.alvo.pieria.api.request`) with lenient
`fromWire` parsing, matching `MemoryType`'s convention.
...
traces
  ...
  │
  ├─ 2. filter          TraceRelevanceFilter (deterministic)
  │
  ├─ 3. persist raw     insertMessages(role="tool", content=canonical summary)
  │                     Survivors only — storing every successful Read would put
  │                     pure noise into MessageFtsChannel. INSERT OR IGNORE over a
  │                     content-addressed id ⇒ idempotent, and MessageFtsChannel
  │                     then covers raw traces for free.
  │
...
- **Skip unchanged outcomes** (`skip-unchanged-outcomes`, default on): drop a trace whose active
  `trace:outcome:<sig>` row already records the same status *and* the same **error digest** — the
  128-bit hash of the last 512 characters of the redacted `error`/`output`, whitespace-collapsed and
  with line/column numbers masked, so a recompile that shifts a stack frame by a line still counts as
  the same outcome. An empty error digests to a fixed sentinel, so success-to-success is compared on
  status alone. Re-writing
...
- `occurred_at` **and** `stated_at` are both the trace's resolved `occurredAt` (§5's fallback chain:
  `endedAt` → `startedAt` → receipt time). `occurred_at` because the command genuinely ran then;
  `stated_at` because supersession ordering reads it (see D5). Neither is `Memory.createdAt`, which is
  store time and would misorder a late-drained spool.
...
**`TraceGraphBuilder`** — emitted entity types are `command`, `tool`, `test`, `file`, and `module`.
The feature description's "build tool" is not a separate type: Gradle, Maven, and npm are `tool`
nodes like any other, distinguished by name rather than by a type that only ever holds three values.
Triples:
...
`ContentId.forTrace(profileId, sessionId, tool, canonicalArgs, status, occurredAt)` — SHA-256
truncated to 128 bits, matching every other id in `ContentId`. `profileId` is in the hash input,
consistent with messages and memories: identical traces coexist across profiles, re-ingest within one
profile is a no-op.

**`canonicalArgs`** is the *redacted, whitespace-collapsed, path-normalized* `args` — deliberately
**not** the `CommandSignature`. The signature strips flags, so two genuinely different invocations
(`./gradlew test` and `./gradlew test --rerun-tasks`) would collide into one id and the second would
be silently dropped. The signature's job is grouping for supersession; the id's job is distinguishing.

**`occurredAt`** is `endedAt`, falling back to `startedAt`, falling back to the daemon's receipt time.
Both DTO timestamps are optional because not every harness reports them, but the fallback chain must
be explicit: a trace with no timestamps that silently took the ingest clock would be indistinguishable
from one that genuinely ran then, and a late-drained spool would misorder against §D5's supersession.
A trace that falls all the way through to receipt time is counted and logged as such.

Raw trace rows use `insertMessages`, which is `INSERT OR IGNORE`. Re-draining a spool that was
already ingested derives nothing new because the derived memories are themselves content-addressed —
identical content yields an identical id, and `insertMemory` dedupes. That guarantee holds
independently of configuration; `skip-unchanged-outcomes` reduces the work done to reach it but is
...
- `TraceSpool` — append/drain round-trip, concurrent appends under lock, growth cap drops oldest,
  retention sweep.
- `TraceMemoryFactory` — payload contract; `stated_at`/`occurred_at` both set from the resolved
  `occurredAt`, including each step of the `endedAt` → `startedAt` → receipt-time fallback.
